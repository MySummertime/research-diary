...
from .network import Node, Arc, TransportTask, TransportNetwork
...
class Path:
    ...
    def from_arc_list(task: Optional[TransportTask], arcs: List[Arc]) -> 'Path':
        """
        从一个有序的弧段列表中构建一个完整的、经过验证的Path对象。
        """
        new_path = Path(task)
        if not arcs:
        ...
        for arc in arcs:
            if arc.start != current_node:
                ...
                raise ValueError(f"[PathFactory] 任务 {task_id} 路径构建失败: 弧段 {arc.start.id}->{arc.end.id} 与前序节点 {current_node.id} 不匹配")
...
class PathFinder:
    ...
    def __init__(self, network: TransportNetwork):
        ...
        self.adj: Dict[str, Dict[str, List[Arc]]] = {
            "road": {},
            "railway": {}
        }
    ...
    def find_all_candidate_paths(self) -> Dict[str, List[Path]]:
        ...
        print(f"路径搜索完成。共为 {len(self.network.tasks)} 个任务找到了候选路径。")
    ...
    def _find_sub_paths(
        ...
        def dfs_recursive(
            ...
            if isinstance(end_type, Node):
                if current_node.id == end_type.id: is_end_node = True
            elif isinstance(end_type, str):
                if current_node.type == end_type: is_end_node = True
            ...
            for arc in self.adj[mode][current_node.id]:
                ...
                if neighbor_node.id not in visited_nodes:
                    ...
                    if isinstance(end_type, Node):
                        if neighbor_node.id == end_type.id: is_neighbor_end_node = True
                    elif isinstance(end_type, str):
                        if neighbor_node.type == end_type: is_neighbor_end_node = True
                    
                    if is_neighbor_end_node or neighbor_node.type == allowed_intermediate_type:
