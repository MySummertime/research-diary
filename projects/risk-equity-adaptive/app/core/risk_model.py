...
class DynamicRiskModel:
    ...
    def _precompute_emergency_response_times(self):
        ...
        setup_time = self.config.get("emergency_setup_time", 0.05)  # t0，假设确定性
        ...
        for node in self.network.nodes:
            ...
            if b_travel == float("inf"):
                ...
                b = b_travel + setup_time  # 最可能总响应时间
                base_extend = b - setup_time  # 可扩展的基础宽度（从 setup_time 到 b）
        ...
        for arc in self.network.arcs:
            ...
            if min_mid_most_likely == float("inf"):
                fuzzy_mid = (self.max_response_time,) * 3
            else:
                b_mid = min_mid_most_likely + setup_time
                base_mid = b_mid - setup_time
