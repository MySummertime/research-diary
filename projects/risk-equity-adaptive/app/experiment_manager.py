...
from .utils.plotter import ParetoPlotter, plot_parallel_coordinates
...
class Experiment:
    ...
    def _backup_experiment_files(self):
        ...
        try:
            shutil.copy(
                self.config_path, os.path.join(self.save_dir, "config_snapshot.json")
            )
    ...
    def setup(self) -> Tuple[TransportNetwork, Evaluator, Dict[str, List[Path]]]:
        ...
        for task in network.tasks:
            ...
            if not paths:
                logging.warning(
                    f"任务 {task.task_id} ( {task.origin.id} -> {task.destination.id} ) 找不到任何合法路径!"
                )
                has_warning = True
            else:
                logging.info(
                    f"     - 任务 {task.task_id}: 找到了 {len(paths)} 条候选路径。"
                )
        if has_warning:
    ...
    def _visualize_network(self):
        ...
        try:
            visualize_network(
                self.network,
                save_path=os.path.join(self.save_dir, "network_topology.png"),
            )
    ...
    def run(self):
        ...
        try:
            self.final_pareto_front = self.algorithm.run(
                callbacks=[generational_logger, file_logger]
            )
    ...
    def analyze_and_report(self):
        ...
        # --- 原始数据
        save_results_json(
            self.final_pareto_front, os.path.join(self.save_dir, "final_archive.json")
        )

        # --- 提取 Rank 0 解
        rank_0_solutions = [
            s for s in self.final_pareto_front if s.rank == 0 and s.is_feasible
        ]
        if rank_0_solutions:
            save_solutions_csv(rank_0_solutions, self.save_dir)
        else:
            logging.warning(
                "未找到任何 Rank 0 可行解，跳过 'PF_solutions.csv' 的生成。"
            )
        ...
        plotter.plot(
            ...
            xlabel="Transportation Risk (people·t)",
            ylabel="Transportation Cost (yuan)",
        )
        ...
        with log_section(clean=True):
            plot_risk_histogram(
                ...
                file_name="final_front_risk_histogram.png",
            )

            analyze_operator_contribution(self.algorithm.op_log)
