...
from core.operators import LoggingCrossover, LoggingMutation, HubSampling, HubCrossover, LocalSearchMutation
...
def get_experiment_config():
    ...
    algorithm_config = {
        ...
        'mutation_prob': 0.5,   # 变异概率，值越大，变异概率越大
        'localsearch_prob': 0.2     # 局部搜索概率，值越大，局部搜索越频繁
    }
...
def main():
    ...
    # --- 3. 执行阶段 (Execution) ---
    logging.info("--- [3/4] 正在配置并运行 Memetic Algorithm (NSGA-II + Local Search)... ---")

    # 3.1 使用代理模式来包装算子
    
    # 1. 实例化自定义的智能算子进行交叉和变异
    # crossover = UniformCrossover(prob=algo_cfg['crossover_prob'])
    crossover = HubCrossover()
    mutation = LocalSearchMutation(problem, global_prob=0.2, local_prob=0.8)

    # 2. 用自定义的 Logging 类来包装它们
    logging_crossover = LoggingCrossover(crossover)
    logging_mutation = LoggingMutation(mutation)
